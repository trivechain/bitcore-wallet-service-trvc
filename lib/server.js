...
var TriveAppService = require('./triveappservice');
...
function WalletService() {
  ...
  this.triveAppService = triveAppService;
  this.notifyTicker = 0;
};
...
WalletService.initialize = function (opts, cb) {
  ...
  function initTriveAppService(cb) {
    if (opts.triveAppService) {
      triveAppService = opts.triveAppService;
      return cb();
    } else {
      var newTriveAppService = new TriveAppService();
      var opts2 = opts.triveAppServiceOpts || {};
      opts2.storage = storage;

      newTriveAppService.init(opts2, function (err) {
        if (err) return cb(err);
        triveAppService = newTriveAppService;
        return cb();
      });
    }
  };
...
WalletService.prototype._validateOutputs = function (opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    if (!checkRequired(output, ['amount'])) {
      if (!(checkRequired(output, ['toAddress']) || checkRequired(output, ['script']))) {
    ...
    return _.map(txs, function (tx) {
      ...
      if (tx.outputs.length || tx.inputs.length) {
        ...
        if (action == 'sent' || action == 'moved') {
          var firstExternalOutput = _.find(outputs, function(o) {
            return !o.isMine && o.address !== undefined && o.address !== "" && o.amount !== 0;
          });
 ...
 * @returns [{Object}] Found apps information
 */
WalletService.prototype.getTriveApps = function (opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['name', 'repo'], cb)) return;

  var networkName = Bitcore.Address(opts.repo).toObject().network;
  opts.bc = self._getBlockchainExplorer(networkName);

  self.triveAppService.getApps(opts, function (err, apps) {
    if (err) return cb(err);
    return cb(null, apps);
  });
};
...
/**
 * Returns TriveApp latest info
 * @param {Object} opts
 * @param {string} opts.addresses - App wallet addresses
 * @returns [{Object}] Found apps information
 */

WalletService.prototype.getTriveAppLatestInfo = function (opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['addresses', 'repo'], cb)) return;

  var networkName = Bitcore.Address(opts.repo).toObject().network;
  opts.bc = self._getBlockchainExplorer(networkName);

  self.triveAppService.getLatest(opts, function (err, info) {
    if (err) return cb(err);
...
/**
 * Find a TriveApp categories
 * @param {Object} opts
 * @param {string} opts.repo - TriveApp repo
 * @returns [{Object}] Found apps information
 */
WalletService.prototype.getTriveAppCategories = function (opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['repo'], cb)) return;

  var networkName = Bitcore.Address(opts.repo).toObject().network;
  opts.bc = self._getBlockchainExplorer(networkName);

  self.triveAppService.getCategories(opts, function (err, categories) {
    if (err) return cb(err);
