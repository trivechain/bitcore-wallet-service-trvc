...
var BizAppService = require('./bizappservice');
...
function WalletService() {
  ...
  this.bizAppService = bizAppService;
  this.notifyTicker = 0;
};
...
WalletService.initialize = function (opts, cb) {
  ...
  function initBizAppService(cb) {
    if (opts.bizAppService) {
      bizAppService = opts.bizAppService;
      return cb();
    } else {
      var newBizAppService = new BizAppService();
      var opts2 = opts.bizAppServiceOpts || {};
      opts2.storage = storage;

      newBizAppService.init(opts2, function (err) {
        if (err) return cb(err);
        bizAppService = newBizAppService;
        return cb();
      });
    }
  };
 ...
 */
WalletService.prototype.getBizApps = function (opts, cb) {
  var self = this;
  ...
  self.bizAppService.getApps(opts, function (err, apps) {
    if (err) return cb(err);
    return cb(null, apps);
  });
};
...
/**
 * Returns BizApp latest info
 * @param {Object} opts
 * @param {string} opts.addresses - App wallet addresses
 * @returns [{Object}] Found apps information
 */

WalletService.prototype.getBizAppLatestInfo = function (opts, cb) {
  var self = this;
  ...
  self.bizAppService.getLatest(opts, function (err, info) {
    if (err) return cb(err);
...
/**
 * Find a BizApp categories
 * @param {Object} opts
 * @param {string} opts.repo - BizApp repo
 * @returns [{Object}] Found apps information
 */
WalletService.prototype.getBizAppCategories = function (opts, cb) {
  var self = this;
  ...
  self.bizAppService.getCategories(opts, function (err, categories) {
    if (err) return cb(err);
