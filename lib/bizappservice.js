...
function extractBizApp(hex, name) {
    let result = false;
    const regex = /(.*?)23626123(([0-9A-Fa-f][0-9A-Fa-f])+?23)(.*)/g;
    const regResults = regex.exec(hex);
    if (regResults.length == 5) {
        const app_name = Buffer.from(regResults[2], 'hex').toString('utf8').slice(0,-1);
        if (app_name.toLowerCase().indexOf(name.toLowerCase()) >= 0) {
            const compressed = regResults[4].match(/.{1,2}/g).join(" ");
...
BizAppService.prototype.getCategories = function (opts, cb) {
    var self = this;
    $.shouldBeFunction(cb);
        ...
        for (let i = 0; i < transactions.length; i++) {
            try {
                ...
                if (tx.vout[1].spentTxId === null && tx.vout.length > 1 && parseFloat(tx.vout[1].value) === VOP_CATEGORIES) {
                    let res = Buffer.from(tx.vout[0].scriptPubKey.hex, 'hex').toString('utf8').substring(4);
