...
BizAppService.prototype.getApps = function (opts, cb) {
    ...
    opts = opts || {};
    opts.bc.getTransactions([opts.repo], 0, 0, function (err, transactions) {
        if (err) return cb(err, null);
        let results = [];
        for (let i = 0; i < transactions.length; i++) {
            try {
                let tx = transactions[i];
                if (tx.vout.length > 1 && parseFloat(tx.vout[1].value) === VOP_PUBLISH) {
...
BizAppService.prototype.getLatest = function (opts, cb) {
    ...
    opts = opts || {};
    opts.bc.getTransactions(opts.addresses, 0, 0, function (err, transactions) {
        if (err) return cb(err, null);
        let results = {};
        for (let i = 0; i < transactions.length; i++) {
            try {
                ...
                if (tx.vout.length > 1 && parseFloat(tx.vout[1].value) === VOP_META) {
                    if (!results[addr]) {
