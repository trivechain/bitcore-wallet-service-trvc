...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v3/triveapp/:repo/discover/:name', function(req, res) {
    var server;
    var opts = {
    ...
    server.getTriveApps(opts, function(err, rates) {
      if (err) return returnError(err, res, req);
      res.json(rates);
    });
  });

  router.post('/v3/triveapp/:repo/latest', function(req, res) {
    var server;
    var opts = {
      addresses: req.body.addrs.split(','),
      repo: req.params['repo']
    };
    try {
      server = getServer(req, res);
    ...
    server.getTriveAppLatestInfo(opts, function(err, info) {
      if (err) return returnError(err, res, req);
      res.json(info);
    });
  });

  router.get('/v3/triveapp/:repo/categories', function(req, res) {
    var server;
    var opts = {
      repo: req.params['repo']
    };
    ...
    server.getTriveAppCategories(opts, function(err, categories) {
      if (err) return returnError(err, res, req);
      res.json(categories);
    });
  });
